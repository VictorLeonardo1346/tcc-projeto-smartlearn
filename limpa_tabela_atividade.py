...
# =============================
@app.route("/professor/criar_questionario", methods=["GET", "POST"])
def criar_questionario():
    ...
    # Corrigido: agora renderiza o template certo
    return render_template("quiz_form.html")

# =============================
# SALVAR QUESTIONÁRIO VIA API
# =============================
...
def salvar_respostas():
    ...
    # Cria/atualiza tabela de Ranking
    ...
    return jsonify({"status": "sucesso", "mensagem": "Respostas salvas com sucesso!"})
...
def aluno_ranking():
    ...
    cur.execute("""SELECT AlunoId, SUM(Pontos) as total
                   FROM Ranking
                   GROUP BY AlunoId
                   ORDER BY total DESC""")
