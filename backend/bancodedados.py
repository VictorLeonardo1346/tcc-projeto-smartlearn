...
class Database:
    ...
    def adicionar_questao(self, questionario_id, enunciado, alternativas, correta, imagem=None):
        self.cursor.execute("""
            INSERT INTO Questoes (
                QuestionarioId, Enunciado, AlternativaA, AlternativaB, AlternativaC,
                AlternativaD, Correta, ImagemPath
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            questionario_id,
    ...
    def listar_questionarios(self):
        self.cursor.execute("""
            SELECT Id, Materia, Titulo, Dificuldade, DataEntrega
            FROM Questionarios
            ORDER BY Id DESC
        """)
    ...
    def buscar_questoes(self, questionario_id):
        self.cursor.execute("""
            SELECT Id, Enunciado, AlternativaA, AlternativaB, AlternativaC,
                   AlternativaD, Correta, ImagemPath
            FROM Questoes
